$(document).ready(function(){
    ...
    $('#updating-db').css({ display: "none" });
    $('#updating').css({display:"none"});

    // data table filters //
    $('#proxies thead tr').clone(true).appendTo( '#proxies thead' );

    $('#proxies thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#proxies').DataTable({select: true, orderCellsTop: true, fixedHeader: false, searching: false});
    ...
    $('#generate-forecast').click(function() {
            ...
            */
            ...
            var metric = document.getElementsByName('metric');
            ...
            var overpredict = $('#overpredict-value').val();
            var underpredict = $('#underpredict-value').val();
            ...
            for (var i=0; i<metric.length; i++) {
             if (metric[i].checked) {
                selected.push(metric[i].value);
                }
            }

            selected.push(underpredict);
            selected.push(overpredict);

            // Determine what is checked and push to selected
            for (var i=0; i<frequencies.length; i++) {
             if (frequencies[i].checked) {
                selected.push(frequencies[i].value);
                }
            }
            // Forecast Settings with Original Data and time frequency
            // settings = [selected,msg,freq];
            settings = [selected, 'msg', 'daily']
            ...
            $('#forecastChart').css({ display: "none" });
            $('#tableau-forecast-visualization').css({ display: "none" });

        }); // End of generate forecast function

    function updatePostgres(e) {
        $('#proxies').css({display:"none"});
        $('#stats-table').css({display:"none"});
        $('#updating-db').css({ display: "block" });                            // Display a loading gif to signify that they model is being built
        $('#updating').css({display:"block"});
        $('#update-widgets').css({display:"block"});
        ...
        Papa.parse(file, {
          ...
          complete: function(results) {
            data = results.data;
            socket.emit('update_data', {data:data});
            }
         });
    }
    ...
    $("#update-db").change(updatePostgres);
    ...
    socket.on("refresh_overview_table", function(msg){

        $('#updating-db').css({ display: "none" });
        $('#proxies').css({display:"block"});
        $('#updating').css({display:"none"});
        $('#stats-table').css({display:"block"});
        window.scrollTo(0, 0);
    })
    ...
    socket.on('render_uploaded_csv_data', function(msg) {
        ...
        */
        ...
        var metric_count = stats_list[5];               // Count time series in data
        ...
        for (var i = 0; i < uids.length; i++) {
            $("#time-series-selection").append('<option value="' + uids[i] + '">' + uids[i] + '</option>');
        }
        ...
        */
        ...
        $('#metrics-count').html(metric_count);
    ...
    socket.on('render_forecast_chart', function(msg) {
        ...
        */
        console.log("You're in render forecast");
        // First, let's hide the loading gif and then display the chart canvas
        $('img#loading').css({ display: "none" });
        $('#myChart').css({ display: "block" });
        $('#forecastChart').css({ display: "block" });
        $('#tableau-forecast-visualization').css({ display: "block" });
        $('#processing').css({display:"none"});
        ...
        var validation_score = arr[10]
        var benchmark_validation_score = arr[11]
        ...
        $('#metrics-rmse').html(validation_score[0]);
        $('#metrics-mae').html(validation_score[1]);
        $('#metrics-cash').html(validation_score[2]);
        $('#metrics-cash2').html(validation_score[2]);
        $('#metrics-benchmark-rmse').html(benchmark_validation_score[0]);
        $('#metrics-benchmark-mae').html(benchmark_validation_score[1]);
        $('#metrics-benchmark-cash').html(benchmark_validation_score[2]);
        $('#metrics-benchmark-cash2').html(benchmark_validation_score[2]);

        console.log("3");
        var percentage_rmse = Math.round(100*(benchmark_validation_score[0] - validation_score[0]) / benchmark_validation_score[0])
        var percentage_mae = Math.round(100*(benchmark_validation_score[1] - validation_score[1]) / benchmark_validation_score[1])
        $('#metrics-benchmark-rmse-percent').html(percentage_rmse);
        $('#metrics-benchmark-mae-percent').html(percentage_mae);
