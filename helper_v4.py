...
def multiForecast(data,forecast_settings,column_headers,freq_val,build_settings):
    ...
    cur = conn.cursor()
    output1 = io.StringIO()
    metrics_table = metrics_table.set_index(metrics_table.index.astype(str) + '_' + session['username'])
    ...
    cur = conn.cursor()
    output1 = io.StringIO()
    output = output.set_index(output.index.astype(str) + '_' + session['username'])
    ...
    cur = conn.cursor()
    output1 = io.StringIO()
    change_table = change_table.set_index(change_table.index.astype(str) + '_' + session['username'])
